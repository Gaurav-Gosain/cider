...
@_cdecl("FMLanguageModelSessionRespondWithOptions")
public func FMLanguageModelSessionRespondWithOptions(
  session: FMLanguageModelSessionRef,
  prompt: UnsafePointer<CChar>,
  temperature: Double,
  maxTokens: Int32,
  ...
  if temperature >= 0 && maxTokens > 0 {
    options = GenerationOptions(temperature: temperature, maximumResponseTokens: Int(maxTokens))
...
@_cdecl("FMLanguageModelSessionStreamResponseWithOptions")
public func FMLanguageModelSessionStreamResponseWithOptions(
  session: FMLanguageModelSessionRef,
  prompt: UnsafePointer<CChar>,
  temperature: Double,
  maxTokens: Int32
...
/// State for a pending tool call — either we have a buffered result
/// (foreignCall completed before the continuation was registered) or
/// a continuation waiting for the result.
enum ToolCallState {
  case waitingForResult(CheckedContinuation<String, any Error>)
...
final class BridgedTool: Tool {
  ...
  let callStates = Mutex<[CUnsignedInt: ToolCallState]>([:])
  ...
  func call(arguments: GeneratedContent) async throws -> String {
    ...
    let callId = nextID()
    // foreignCall is synchronous — it may call FMBridgedToolFinishCall
    // before we register the continuation below. So we call it first,
    // then check if the result was already buffered.
    foreignCall(FMGeneratedContentRef(Unmanaged.passRetained(arguments).toOpaque()), callId)

    // Check if the result arrived synchronously during foreignCall.
    let buffered: String? = callStates.withLock { states in
      if case .resultReady(let result) = states[callId] {
        states.removeValue(forKey: callId)
        return result
      }
      return nil
    }
    if let result = buffered {
      return result
    }

    // Result hasn't arrived yet — register a continuation and wait.
    return try await withCheckedThrowingContinuation { continuation in
      callStates.withLock { states in
        // Double-check: result may have arrived between our check above
        // and registering the continuation.
        if case .resultReady(let result) = states[callId] {
          states.removeValue(forKey: callId)
          continuation.resume(returning: result)
        } else {
          states[callId] = .waitingForResult(continuation)
        }
      }
    }
  }
...
public func FMBridgedToolFinishCall(
  ...
  bridgedTool.callStates.withLock { states in
    if case .waitingForResult(let continuation) = states[callId] {
      // Continuation already registered — resume it.
      states.removeValue(forKey: callId)
      continuation.resume(returning: result)
    } else {
      // foreignCall returned synchronously before the continuation
      // was registered — buffer the result.
      states[callId] = .resultReady(result)
    }
  }
}
